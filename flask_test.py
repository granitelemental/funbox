...
class TestPost(unittest.TestCase):
    ...
    def test_post_ok(self):
        ...
        self.assertTrue(response.json(), response.status_code) == (
            {"status": "ok"}, 200)

    def test_post_no_data(self):
        response = requests.post(url=self.url, json=None)
        self.assertTrue(response.json(), response.status_code) == (
            {"status": "there is no data"}, 400)
    ...
    def test_post_not_json_format(self):
        ...
        for d in data:
            ...
            self.assertTrue(response.json(), response.status_code) == (
                {"status": "Data is not a dict"}, 400)

    def test_post_not_no_links(self):
        response = requests.post(url=self.url, json={})
        self.assertTrue(response.json(), response.status_code) == (
            {"status": "Trere is no field 'links' in data"}, 400)
    ...
    def test_post_links_not_list(self):
        ...
        for d in data:
            response = requests.post(url=self.url, json=d)
            self.assertTrue(response.json(), response.status_code) == (
                {"status": "'links' is not a list"}, 400)

    def test_post_links_empty(self):
        json = {"links": []}
        response = requests.post(url=self.url, json=json)
        self.assertTrue(response.json(), response.status_code) == (
            {"status": "'links' list is empty"}, 400)
...
class TestGet(unittest.TestCase):
    ...
    def test_valid_from_to(self):
        ...
        for d in data:
            ...
            self.assertTrue(response.json(), response.status_code) == (
                {"status": "start/end is not a digit"}, 400)
    ...
    def test_from_greater_than_to(self):
        ...
        self.assertTrue(response.json(), response.status_code) == (
            {"status": "'from' is greater than 'to'"}, 400)
